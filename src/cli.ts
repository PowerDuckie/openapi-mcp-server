...
program
  .name("openapi-mcp")
  .description("Convert OpenAPI documents into MCP services.")
  .version("1.0.0");
...
program
  ...
  .option(
    "--upstream-header <headerPair>",
    "Repeatable upstream header",
    parseHeaderOption,
    {},
  )
  ...
  .action(async (options) => {
    if (options.transport === "stdio") {
      if (!options.spec)
        throw new Error("The --spec option is required in stdio mode.");
